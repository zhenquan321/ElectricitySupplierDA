...
namespace IWSBot.Queries
{
    public class IW2SBaiduQuery
    {
        ...
        void GetLinks(string link, IW2S_BaiduCommend tsk, List<IW2S_ExcludeKeyword> excludedKeywords)
        {
            ...
            string[] searchKeywords = tsk.CommendKeyword.GetLower().RemoveSpace().Split(';');
            ...
            while (!string.IsNullOrEmpty(link) && quried_pages <= 20)
            {
                ...
                foreach (var tag in propContents)
                {
                    ...
                    string abs = RemoveInivalidChar(tag.SubAfter("</h3>").SubBefore("</a").GetTxtFromHtml2().RemoveSpace().GetLower());
