var WC_dashboard_ctr = myApp.controller("WC_dashboard_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, $filter, $timeout, myApplocalStorage) {
	...
	$scope.D_lineChart_Change = function() {
		...
		//指定图表的配置项和数据
		var myChart = echarts.init(document.getElementById('WC_lineChart'));
	...
	$scope.D_lineChart = function(timeValue, config) {
		...
		url = "/api/Media/GetTimeLinkCount?categoryIds=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId + "&startTime=" + $scope.startTime + "&endTime=" + $scope.endTime + "&percent=" + $scope.data.percent + "&topNum=" + $scope.data.topNum + "&sumNum=" + 25 + "&timeInterval=" + timeInterval;
		var q = $http.get(url);
		q.success(function(response, status) {
	...
	$scope.D_PieChart = function(date) {
			...
			var myChart = echarts.init(document.getElementById('WC_PieChart'));
			console.log(date);
			var data_n = [];
			var date_a = [];
	...
	$scope.D_GetBubbleList = function() {
		...
		q.success(function(response, status) {
			...
			var url = "/api/Media/GetDomainStatis?categoryIds=" + $scope.categoryId + "&projectId=" + $rootScope.getProjectId;
			var q = $http.get(url);
			q.success(function (response, status) {
			    console.log(response);
	...
	$scope.quanzhongtu = function(treeNodeId) {
	    ...
	    var url = "/api/Jieba/MediaExtract?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
		var q = $http.get(url);
		q.success(function(response, status) {
			console.log(response);
			$("#WC_wordcloud1").html(response);
			//$('#wordcloud1').before('<h4 style="font-weight:700;margin:17px 0 0 17px;">关键词权重图</h4>');
			$("#WC_wordcloud1").awesomeCloud({
				"size": {
					"grid": 3, // 字间距
					"factor": 3, // 字体大小  0位自动
	...
	$scope.cipintongji = function(treeNodeId) {
	    var url = "/api/Jieba/MediaFrequency?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
		var q = $http.get(url);
		q.success(function(response, status) {
			console.log(response);
	...
	$scope.cipintu = function(n, nc, v, vc) {
		//词频柱状图
		//名词
		var myChart = echarts.init(document.getElementById('WC_WordFrequencyM'));
		...
		var myChart = echarts.init(document.getElementById('WC_WordFrequencyD'));
	...
	$scope.Links = { page: 1, pagesize: 10 };
	//搜索发表时间显示链接表格
	$scope.searchLinkByTime = function(time) {
		...
		if (re.test($scope.data.searchTime) || $scope.data.searchTime == '') {
			...
			var url = "/api/Media/GetTimeLinkList?prjId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId1 + "&pubTime=" + time + "&page=" + ($scope.Links.page - 1) + "&pagesize=" + $scope.Links.pagesize;
			var q = $http.get(url);
			q.success(function(response, status) {
				console.log(response);
				if (response.Count == 0) {
	...
	$scope.GetTimeLinkList = function(treeNodeId) {
	    var url = "/api/Media/GetTimeLinkList?prjId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId + "&pubTime=" + "&page=" + ($scope.Links.page-1) + "&pagesize=" + $scope.Links.pagesize;
		var q = $http.get(url);
		q.success(function(response, status) {
		    console.log(response);
		    $scope.TLcount = response.Count;
	...
	$scope.getkeyword = function(treeId, treeNode) {
			...
			var url = "/api/Media/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + treeNode.id + "&status=" + $scope.getNotRemovedKw;
			var q = $http.get(url);
			q.success(function(response, status) {
	...
	$scope.getkeyword1 = function(id) {
		var url = "/api/Media/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + id + "&status=" + $scope.getNotRemovedKw;
		var q = $http.get(url);
		q.success(function(response, status) {
			$scope.GetAllKeywordCategory_list = response;
	...
	$scope.cancelCheckedword = function() {
			if (confirm("确定要删除记录吗？")) {
				...
				var IsRemoved = true;
				var url = "/api/Media/SetKeywordStatus?categoryId=" + checkedId + "&status=" + IsRemoved;
				var q = $http.get(url);
				q.success(function(response, status) {
    ...
    //打开分享文章
	$scope.getShareArticle = function (name, nameId) {
	    var CP_scope = $scope;
	    CP_scope.gzhname = name;
	    CP_scope.nameId = nameId;
	    CP_scope.categoryIds = $scope.GNScategoryIds;
	    CP_scope.projectId = $rootScope.getProjectId;
	    var frm = $modal.open({
	        templateUrl: 'Scripts/app/views/Dashboard/modal/getShareArticle.html',
	        controller: getShareArticle_ctr,
	        scope: CP_scope,
	        // label: label,
	        keyboard: false,
	        backdrop: 'static',
    ...
    //打开关键词文章
	$scope.getKWSArticle = function (name, nameId) {
	    var CP_scope = $scope;
	    CP_scope.gzhname = name;
	    CP_scope.nameId = nameId;
	    CP_scope.categoryIds = $scope.GNScategoryIds;
	    CP_scope.projectId = $rootScope.getProjectId;
	    var frm = $modal.open({
	        templateUrl: 'Scripts/app/views/Dashboard/modal/getKWSArticle.html',
	        controller: getKWSArticle_ctr,
	        scope: CP_scope,
	        // label: label,
	        keyboard: false,
	        backdrop: 'static',
    ...
    //查看公众号详情
	$scope.GetNameInfoM = function (nameId) {
	    var CP_scope = $scope;
	    CP_scope.nameId = nameId;
	    var frm = $modal.open({
	        templateUrl: 'Scripts/app/views/Dashboard/modal/GetGZHInfo.html',
	        controller: GetGZHInfo_ctr,
	        scope: CP_scope,
	        // label: label,
	        keyboard: false,
	        backdrop: 'static',
