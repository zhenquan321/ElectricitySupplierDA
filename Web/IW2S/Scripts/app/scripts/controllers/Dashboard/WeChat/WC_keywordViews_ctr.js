var WC_keywordViews_ctr = myApp.controller("WC_keywordViews_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, myApplocalStorage) {
    ...
    $scope.getkeywords = function () {
        ...
        var url = "/api/Media/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.getkeywords1 = function (treeNodeId) {
        var url = "/api/Media/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            $scope.GetAllKeywordCategory_list = response;
        });
    ...
    $scope.GetBaiduLevelLinksView = function () {
        ...
        var urls = "api/Media/GetLevelLinksView";
        var q = $http.post(
            urls,
    ...
    $scope.SetLinkStatus = function (id, type) {
        ...
        if (type == "delete") {
            ...
            if (isStar != true) {
                var url = "/api/Media/SetLinkStatus?id=" + $scope.id + "&status=" + "&projectId=" + $rootScope.getProjectId;
                var q = $http.get(url);
                q.success(function (response, status) {
                    //console.log(response);
                    console.log('iw2s_ctr>SetLinkStatus');
    ...
    $scope.SetLinkInfriType = function (infriType) {
        if ($scope.ChkId.length == 0) {
            ...
            $scope.ChkId1 = $scope.ChkId.join(";");
            var url = "/api/Media/SetLinkInfriType?id=" + $scope.ChkId1 + "&infriType=" + infriType + "&projectId=" + $rootScope.getProjectId
            var q = $http.get(url);
            q.success(function (response, status) {
    ...
    $scope.ExportLevelLinks = function () {
        ...
        $http({
            ...
            url: "/api/Media/ExportLevelLinks"
        })
