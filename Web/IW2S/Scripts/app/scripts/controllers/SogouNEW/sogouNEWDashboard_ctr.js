var sogouNEWDashboard_ctr = myApp.controller("sogouNEWDashboard_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, $filter, myApplocalStorage) {
    ...
    $scope.GetTreeData = function () {
        var url = "/api/sogou/GetAllFenZhu?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
    ...
    $scope.GetD3TreeData = function () {
        var url = "/api/sogou/GetAllGroupTreeUrl?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>GetTreeData');
            //console.log(response);
    ...
    $scope.D_lineChart = function () {
        var url = "/api/sogou/GetTimeLinkCount?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            ...
            //指定图表的配置项和数据
            var myChart = echarts.init(document.getElementById('D_lineChartSG'));
    ...
    $scope.D_GetBubbleList = function () {
        var url = "/api/sogou/GetDomainStatis?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>GetBubbleList2');
            console.log(response);
            var myChart = echarts.init(document.getElementById('D_GetBubbleListSG'));
    ...
    $scope.quanzhongtu = function (treeNodeId) {
        ...
        var url = "/api/Jieba/SGExtract?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log(response);
            $("#wordcloud1SG").html(response);
            //$('#wordcloud1').before('<h4 style="font-weight:700;margin:17px 0 0 17px;">关键词权重图</h4>');
            $("#wordcloud1SG").awesomeCloud({
                "size" : {
                    "grid" : 3, // 字间距
                    "factor" : 1, // 字体大小  0位自动
    ...
    $scope.cipintongji = function (treeNodeId) {
        var url = "/api/Jieba/SGFrequency?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log(response);
    ...
    $scope.cipintu = function (n, nc, v, vc) {
        //词频柱状图
        //名词
        var myChart = echarts.init(document.getElementById('WordFrequencyMSG'));
        ...
        var myChart = echarts.init(document.getElementById('WordFrequencyDSG'));
    ...
    $scope.getkeyword = function (treeId, treeNode) {
        ...
        var url = "/api/sogou/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + treeNode.id + "&status=" + $scope.getNotRemovedKw;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.getkeyword1 = function (id) {
        var url = "/api/sogou/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + id + "&status=" + $scope.getNotRemovedKw;
        var q = $http.get(url);
        q.success(function (response, status) {
            $scope.GetAllKeywordCategory_list = response;
    ...
    $scope.GetBaiduLevelLinks = function () {
        ...
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('iw2s_sogou_ctr>GetBaiduLeveLinks');
    ...
    $scope.cancelCheckedword = function () {
        if (confirm("确定要删除记录吗？")) {
            ...
            var IsRemoved = true;
            var url = "api/sogou/SetKeywordStatus?categoryId=" + checkedId + "&status=" + IsRemoved;
            var q = $http.get(url);
            q.success(function (response, status) {
