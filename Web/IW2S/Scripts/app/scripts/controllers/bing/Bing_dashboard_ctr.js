var Bing_dashboard_ctr = myApp.controller("Bing_dashboard_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, $filter, myApplocalStorage) {
    ...
    $scope.GetTreeData = function () {
      var url = "/api/Bing/GetAllFenZhu?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
      var q = $http.get(url);
    ...
    $scope.D_lineChart = function () {
        var url = "/api/Bing/GetTimeLinkCount?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId + "&startTime=" + $scope.startTime + "&endTime=" + $scope.endTime + "&percent=" + $scope.data.percent + "&topNum=" + $scope.data.topNum;
      var q = $http.get(url);
      q.success(function (response, status) {
    ...
    $scope.D_GetBubbleList = function () {
        var url = "api/Bing/GetAllDomainCategory?prjId=" + $rootScope.getProjectId;
      var q = $http.get(url);
      q.success(function (response, status) {
      ...
      var url = "/api/Bing/GetDomainStatis?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId;
      var q = $http.get(url);
      q.success(function (response, status) {
          console.log(response);
...
//搜索发表时间显示链接表格
    $scope.searchLinkByTime = function (time) {
      ...
      if (re.test($scope.data.searchTime)) {
        ...
        var url = "/api/Bing/GetTimeLinkList?usr_id=" + $rootScope.userID + "&prjId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId1 + "&pubTime=" + time;
        var q = $http.get(url);
        q.success(function (response, status) {
          console.log(response);
          if (response.Count == 0) {
...
//显示链接表格
    $scope.GetTimeLinkList = function (treeNodeId) {
        var url = "/api/Bing/GetTimeLinkList?usr_id=" + $rootScope.userID + "&prjId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId + "&pubTime=";
      var q = $http.get(url);
      q.success(function (response, status) {
        console.log(response);
        $scope.TimeLinkList = response.Result;
...
//选择分组后加载关键词
    $scope.getkeyword = function (treeId, treeNode) {
      ...
      var url = "/api/Bing/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + treeNode.id + "&status=" + $scope.getNotRemovedKw;
      var q = $http.get(url);
      q.success(function (response, status) {
...
//未排除的关键词
    $scope.getkeyword1 = function (id) {
        var url = "/api/Bing/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + id + "&status=" + $scope.getNotRemovedKw;
      var q = $http.get(url);
      q.success(function (response, status) {
        $scope.GetAllKeywordCategory_list = response;
...
//2.3.1 百度搜索记录的结果
    ...
    $scope.GetBaiduLevelLinks = function () {
        var url = "/api/Bing/GetLevelLinks?user_id=" + $rootScope.userID + "&categoryId=" + $rootScope.categoryId + "&projectId=" + $rootScope.getProjectId + "&keywordId=" + $rootScope.BaidukeywordId + "&Title=" + encodeURIComponent($scope.Title) +
...
//排除关键词
    ...
    $scope.cancelCheckedword = function () {
      if (confirm("确定要删除记录吗？")) {
        ...
        var IsRemoved = true;
        var url = "api/Bing/SetKeywordStatus?categoryId=" + checkedId + "&status=" + IsRemoved;
        var q = $http.get(url);
        q.success(function (response, status) {
