var keywordViews_ctr = myApp.controller("keywordViews_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, myApplocalStorage) {
    ...
    $scope.getkeywords = function () {
        ...
        var url = "/api/Keyword/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.getkeywords1 = function (treeNodeId) {
        var url = "/api/Keyword/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            $scope.GetAllKeywordCategory_list = response;
        });
    ...
    $scope.GetBaiduLevelLinksView = function () {
        ...
        var urls = "api/Keyword/GetLevelLinksView";
        var q = $http.post(
            urls,
    ...
    //5.批量刪除連接
    $scope.DelLinkStatus2 = function () {
        if ($scope.ChkId.length == 0) {
            alert('您还未勾选链接详情');
        }
        $scope.ChkId1 = $scope.ChkId.join(";");
        $scope.SetLinkStatus($scope.ChkId1,'delete');
    }
    ...
    $scope.SetLinkStatus = function (id, type) {
        ...
        if (type == "delete") {
            ...
            if (isStar != true) {
                var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&status=" + "&projectId=" + $rootScope.getProjectId;
                var q = $http.get(url);
                q.success(function (response, status) {
                    //console.log(response);
                    console.log('iw2s_ctr>SetLinkStatus');
    ...
    $scope.SetLinkInfriType = function (infriType) {
        if ($scope.ChkId.length == 0) {
            ...
            $scope.ChkId1 = $scope.ChkId.join(";");
            var url = "/api/Keyword/SetLinkInfriType?id=" + $scope.ChkId1 + "&infriType=" + infriType + "&projectId=" + $rootScope.getProjectId
            var q = $http.get(url);
            q.success(function (response, status) {
