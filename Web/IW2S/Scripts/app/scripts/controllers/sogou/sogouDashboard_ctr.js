var sogouDashboard_ctr = myApp.controller("sogouDashboard_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, $filter, myApplocalStorage) {
    ...
    $scope.GetTreeData = function () {
        var url = "/api/WXSource/GetAllFenZhu?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>GetTreeData');
    ...
    $scope.GetD3TreeData = function () {
        var url = "/api/WXSource/GetAllGroupTreeUrl?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>GetTreeData');
            //console.log(response);
    ...
    $scope.GetData = function () {
        var url = "/api/WXSource/GetKeywordBygroup?user_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>GetData');
            if (response.Error) {
    ...
    $scope.D_lineChart = function () {
        var url = "/api/WXSource/GetTimeLinkCount?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>lineChart1');
            console.log(response);
            //基于准备好的dom，初始化echarts实例
            //指定图表的配置项和数据
            var myChart = echarts.init(document.getElementById('D_lineChartWX'));
    ...
    $scope.D_GetBubbleList = function () {
        var url = "/api/WXSource/GetDomainStatis?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log('sogouDashboard_ctr>GetBubbleList2');
            console.log(response);
            var myChart = echarts.init(document.getElementById('D_GetBubbleListWX'));


            var data1 = [];

            for (var i = 0; i < response.length; i++) {
                data1.push([response[i].Count, response[i].RankTotal, (response[i].KeywordTotal / 20 + 0.2), response[i].Domain, response[i].PublishRatio]);
            }
            var schema = [
                {index: 0, text: '域名'},
                {index: 1, text: '关键词数'},
    ...
    $scope.quanzhongtu = function (treeNodeId) {
        ...
        var url = "/api/Jieba/WXExtract?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log(response);
            $("#wordcloud1WX").html(response);
            //$('#wordcloud1').before('<h4 style="font-weight:700;margin:17px 0 0 17px;">关键词权重图</h4>');
            $("#wordcloud1WX").awesomeCloud({
                "size" : {
                    "grid" : 3, // 字间距
                    "factor" : 1, // 字体大小  0位自动
    ...
    $scope.cipintongji = function (treeNodeId) {
        var url = "/api/Jieba/WXFrequency?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            console.log(response);
    ...
    $scope.cipintu = function (n, nc, v, vc) {
        //词频柱状图
        //名词
        var myChart = echarts.init(document.getElementById('WordFrequencyMWX'));
        ...
        var myChart = echarts.init(document.getElementById('WordFrequencyDWX'));
    ...
    $scope.getkeyword = function (treeId, treeNode) {
        ...
        var url = "/api/WXSource/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + treeNode.id + "&status=" + $scope.getNotRemovedKw;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.getkeyword1 = function (id) {
        var url = "/api/WXSource/GetFenleiKeywords?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&treeNodeId=" + id + "&status=" + $scope.getNotRemovedKw;
        var q = $http.get(url);
        q.success(function (response, status) {
            $scope.GetAllKeywordCategory_list = response;
    ...
    $scope.GetBaiduLevelLinks = function () {
        var url = "/api/WXSource/GetLevelLinks?user_id=" + $rootScope.userID + "&categoryId=" + $rootScope.categoryId + "&projectId=" + $rootScope.getProjectId + "&keywordId=" + $rootScope.BaidukeywordId + "&Title=" + $scope.Title +
        ...
        q.success(function (response, status) {
            ...
            if (response != null) {
                $rootScope.WXresultList = response.Result;
    ...
    $scope.cancelCheckedword = function () {
        if (confirm("确定要删除记录吗？")) {
            ...
            var IsRemoved = true;
            var url = "api/WXSource/SetKeywordStatus?categoryId=" + checkedId + "&status=" + IsRemoved;
            var q = $http.get(url);
            q.success(function (response, status) {
