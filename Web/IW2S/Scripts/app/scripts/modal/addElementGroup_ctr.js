var addElementGroup_ctr = function ($scope, $modalInstance, $rootScope, $http, $timeout) {
    ...
    $scope.ExcludeBaiduKeyword = function (id) {
        if (confirm("确定要删除这个关键词吗？")) {
            var url = "api/Keyword/ExcludeKeyword?keywordIds=" + id + "&projectId=" + $rootScope.getProjectId;
            var q = $http.get(url);
            q.success(function (response, status) {
                if (response.IsSuccess == true) {
