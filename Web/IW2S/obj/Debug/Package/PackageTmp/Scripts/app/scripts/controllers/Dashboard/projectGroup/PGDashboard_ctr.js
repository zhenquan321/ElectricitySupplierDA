var PGDashboard_ctr = myApp.controller("PGDashboard_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $interval, $filter, $timeout, myApplocalStorage) {
        $scope.zNodes = [];
        $scope.categoryId = "";
        ...
        $scope.GetTreeData = function () {
            var url = "/api/ProCategory/GetAllFenZhu?userId=" + $rootScope.userID + "&categoryId=" + $rootScope.getProjectGroupId;
            var q = $http.get(url);
        ...
        $scope.D_lineChart = function (timeValue, config) {
            ...
            url = "/api/ProCategory/GetTimeLinkCount?categoryId=" + $scope.categoryId + "&prjId=" + $rootScope.getPGPIdS + "&startTime=" + $scope.startTime + "&endTime=" + $scope.endTime + "&percent=" + $scope.data.percent + "&topNum=" + $scope.data.topNum + "&sumNum=" + 25 + "&timeInterval=" + timeInterval;
            var q = $http.get(url);
            q.success(function (response, status) {
        ...
        $scope.searchLinkByTime = function (time) {
            ...
            if (re.test($scope.data.searchTime) || $scope.data.searchTime == '') {
                ...
                var url = "/api/ProCategory/GetTimeLinkList?categoryId=" + $scope.treeNodeId1 + "&pubTime=" + time;
                var q = $http.get(url);
                q.success(function (response, status) {
                    console.log(response);
                    if (response.Count == 0) {
        ...
        $scope.GetTimeLinkList = function (treeNodeId) {
            var url = "/api/ProCategory/GetTimeLinkList?categoryId=" + treeNodeId + "&pubTime=";
            var q = $http.get(url);
            q.success(function (response, status) {
                console.log(response);
                $scope.TimeLinkList = response.Result;
