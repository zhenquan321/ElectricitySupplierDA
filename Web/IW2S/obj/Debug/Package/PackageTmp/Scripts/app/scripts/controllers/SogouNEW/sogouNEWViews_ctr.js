var sogouNEWViews_ctr = myApp.controller("sogouNEWViews_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, myApplocalStorage) {
    ...
    $scope.getkeywords = function () {
        ...
        var url = "/api/sogou/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.getkeywords1 = function (treeNodeId) {
        var url = "/api/sogou/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            $scope.GetAllKeywordCategory_list = response;
        });
    ...
    $scope.GetBaiduLevelLinksView = function () {
        ...
        var urls = "api/sogou/GetLevelLinksView";
        var q = $http.post(
            urls,
    ...
    $scope.SetLinkStatus = function (id, type) {
        ...
        if (type == "delete") {
            if (confirm("确定要删除这条记录吗？")) {
                var url = "/api/sogou/SetLinkStatus?id=" + $scope.id + "&status=" + 2 + "&user_id=" + $rootScope.userID;
                var q = $http.get(url);
                q.success(function (response, status) {
            ...
            if (isStar != true) {
                var url = "/api/sogou/SetLinkStatus?id=" + $scope.id + "&status=" + "&user_id=" + $rootScope.userID;
                var q = $http.get(url);
                q.success(function (response, status) {
                    //console.log(response);
                    console.log('iw2s_ctr>SetLinkStatus');
                ...
                var url = "/api/sogou/SetLinkStatus?id=" + $scope.id + "&status=" + 1 + "&user_id=" + $rootScope.userID;
                var q = $http.get(url);
                q.success(function (response, status) {
                    console.log('iw2s_ctr>SetLinkStatus');
                    //console.log(response);
    ...
    $scope.SetLinkInfriType = function (infriType) {
        if ($scope.ChkId.length == 0) {
            ...
            var url = "/api/sogou/SetLinkInfriType?id=" + $scope.ChkId1 + "&infriType=" + infriType + "&user_id=" + $rootScope.userID;
            var q = $http.get(url);
            q.success(function (response, status) {
    ...
    $scope.ExportLevelLinks = function () {
        ...
        $http({
            ...
            url: "/api/sogou/ExportLevelLinks"
        })
