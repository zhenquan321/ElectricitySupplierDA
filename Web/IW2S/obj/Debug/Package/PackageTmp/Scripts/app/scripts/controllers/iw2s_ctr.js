var iw2s_ctr = myApp.controller("iw2s_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, myApplocalStorage) {
    ...
    //        var url = "/api/Keyword/insertKeyword?user_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&keywords=" + $scope.searchInput + "&isCommend=false";
    //        var q = $http.get(url);
    ...
    //选择推荐关键词弹框
    $scope.SelectKWFun = function (response) {
        var SK_scope = $rootScope.$new();
        SK_scope.TJKws = response;

        var frm = $modal.open({
            templateUrl: 'Scripts/app/views/Dashboard/modal/SelectKws.html',
            controller: SelectKws_ctr,
            scope: SK_scope,
            // label: label,
            keyboard: false,
            backdrop: 'static',
    ...
    $scope.GetBaiduKeyword = function () {
        ...
        var url = "/api/Keyword/GetBaiduKeyword?user_id=" + $rootScope.userID + "&page=" + $scope.pageBaidu + "&projectId=" + $rootScope.getProjectId + "&pagesize=" + $rootScope.pagesizeBaidu;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.GetBaiduLevelLinks = function () {
        var url = "/api/Keyword/GetLevelLinks?user_id=" + $rootScope.userID + "&categoryId=" + $rootScope.categoryId + "&projectId=" + $rootScope.getProjectId + "&keywordId=" + $rootScope.BaidukeywordId + "&Title=" + $scope.Title +
    ...
    $scope.SetLinkStatus = function (id, type) {
        ...
        if (type == "delete") {
            if (confirm("确定要删除这条记录吗？")) {
                var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&status=" + 2 + "&projectId=" + $rootScope.getProjectId;
                var q = $http.get(url);
                q.success(function (response, status) {
            ...
            if (isStar != true) {
                var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&projectId=" + $rootScope.getProjectId + "&status=";
                var q = $http.get(url);
                q.success(function (response, status) {
                    //console.log(response);
                    console.log('iw2s_ctr>SetLinkStatus');
                ...
                var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&status=" + 1 + "&projectId=" + $rootScope.getProjectId;
                var q = $http.get(url);
                q.success(function (response, status) {
                    console.log('iw2s_ctr>SetLinkStatus');
                    //console.log(response);
    ...
    $scope.ExcludeBaiduKeyword = function (_id) {
        if (confirm("确定要删除这个关键词吗？")) {
            var url = "api/Keyword/ExcludeKeyword?keywordIds=" + _id + "&projectId=" + $rootScope.getProjectId;
            var q = $http.get(url);
            q.success(function (response, status) {
    ...
    $scope.GetAllKeywordCategory = function () {
        var url = "/api/Keyword/GetAllKeywordCategory?user_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.SetLinkInfriType = function (id, infriType) {
        ...
        $scope.link1ID = id;
        var url = "/api/Keyword/SetLinkInfriType?id=" + $scope.link1ID + "&infriType=" + $scope.infriType + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
        q.success(function (response, status) {
