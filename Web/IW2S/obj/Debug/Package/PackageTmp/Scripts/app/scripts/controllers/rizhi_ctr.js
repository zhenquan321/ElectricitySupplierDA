var rizhi_ctr = myApp.controller("rizhi_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, myApplocalStorage) {
    ...
    //获取项目id
    if ($rootScope.getProjectId == undefined || $rootScope.getProjectId == null) {
        $rootScope.getProjectId = $location.search().porjectid;
        $rootScope.userID = "";
    ...
    $scope.ExcludeBaiduKeyword = function (_id) {
        if (confirm("确定要删除这条记录吗？")) {
            ...
            var urls = "api/Keyword/ExcludeBaiduKeyword";
            var q = $http.post(
                urls,
    ...
    $rootScope.GetQRCode = function () {
        var QRCode_url = $location.host() + ":" + $location.port() + "/#/";
        QRCode_url = QRCode_url + $scope.url;
        QRCode_url = QRCode_url + "?porjectid=" + $rootScope.getProjectId
        $scope.anaItem = {
            url: QRCode_url,
            baseurl: 'http://' + window.location.host
        };
