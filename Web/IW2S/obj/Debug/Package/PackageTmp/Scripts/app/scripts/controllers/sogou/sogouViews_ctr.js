var sogouViews_ctr = myApp.controller("sogouViews_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal, myApplocalStorage) {
    ...
    $scope.getkeywords = function () {
        ...
        var url = "/api/WXSource/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + $scope.treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
    ...
    $scope.getkeywords1 = function (treeNodeId) {
        var url = "/api/WXSource/GetFenleiKeywordsView?usr_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&categoryId=" + treeNodeId;
        var q = $http.get(url);
        q.success(function (response, status) {
            $scope.GetAllKeywordCategory_list = response;
        });
    ...
    $scope.resultFilterlink = function () {
        $scope.categoryId = $scope.treeNodeId;
...
//2.3.1 百度搜索记录的结果（含链接标签）
    ...
    $scope.GetBaiduLevelLinksView = function () {
        ...
        var urls = "api/WXSource/GetLevelLinksView";
        var q = $http.post(
            urls,
    ...
    $scope.SetLinkInfriType = function (infriType) {
        if ($scope.ChkId.length == 0) {
            ...
            var url = "/api/WXSource/SetLinkInfriType?id=" + $scope.ChkId1 + "&infriType=" + infriType + "&user_id=" + $rootScope.userID;
            var q = $http.get(url);
            q.success(function (response, status) {
    ...
    $scope.ExportLevelLinks = function () {
        ...
        $http({
            ...
            url: "/api/WXSource/ExportLevelLinks"
        })
