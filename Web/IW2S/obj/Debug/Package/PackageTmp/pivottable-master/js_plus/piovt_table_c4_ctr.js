var piovt_table_c4_ctr = myApp.controller("piovt_table_c4_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $interval, $filter, $timeout) {
    ...
    $scope.GetCase = function () {
        var url = "/api/Media/GetLinkReport?userId=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId;
        var q = $http.get(url);
    ...
    $scope.pottle_table = function () {
        ...
        $("#output").pivotUI($scope.GetCase_list, {
            ...
            //rows: ["行业大类"],
            cols: [],
            rows: ["公众号", '文章标题', '相关文章数', '阅读数', '评论数', '文章影响力'],
            vals: [],
