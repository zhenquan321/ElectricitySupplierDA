var rizhi_ctr = myApp.controller("rizhi_ctr", function ($scope, $rootScope, $http, $location, $window, $cookieStore, $modal) {
    ...
    if ($rootScope.getProjectId == undefined || $rootScope.getProjectId==null) {
        ...
        $scope.keywordsSearch = function () {
            if ($scope.searchInput == "" || $scope.searchInput == null) {
                ...
                var url = "/api/Keyword/insertKeyword?user_id=" + $rootScope.userID + "&projectId=" + $rootScope.getProjectId + "&keywords=" + $scope.searchInput;
                var q = $http.get(url);
        ...
        $scope.SetLinkStatus = function (id, type) {
            ...
            if (type == "delete") {
                if (confirm("确定要删除这条记录吗？")) {
                    var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&status=" + 2 + "&user_id=" + $rootScope.userID;
                    var q = $http.get(url);
                    q.success(function (response, status) {
                ...
                if (isStar != true) {
                    var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&user_id=" + $rootScope.userID + "&status=";
                    var q = $http.get(url);
                    q.success(function (response, status) {
                        //console.log(response);
                        console.log('iw2s_ctr>SetLinkStatus');
                    ...
                    var url = "/api/Keyword/SetLinkStatus?id=" + $scope.id + "&status=" + 1 + "&user_id=" + $rootScope.userID;
                    var q = $http.get(url);
                    q.success(function (response, status) {
                        console.log('iw2s_ctr>SetLinkStatus');
                        //console.log(response);
        ...
        $scope.SetLinkInfriType = function (id, infriType) {
            ...
            $scope.link1ID = id;
            var url = "/api/Keyword/SetLinkInfriType?id=" + $scope.link1ID + "&infriType=" + $scope.infriType + "&user_id=" + $rootScope.userID;
            var q = $http.get(url);
            q.success(function (response, status) {
